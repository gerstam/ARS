...
if game.PlaceId == 286090429 then
    ...
    AssistsGroupbox:AddToggle('AutoReload', {
        ...
        Callback = function(Value)
            if Value then
                RunService:BindToRenderStep("AutoReload", 101, function()
                    if Toggles.AutoReload.Value then
                        -- Arsenal auto-reload logic
                ...
                RunService:UnbindFromRenderStep("AutoReload")
            end
        end
    })
    ...
    InfoGroupbox:AddLabel('NSHUB Arsenal Script')
    InfoGroupbox:AddLabel('Version: 1.0')
    InfoGroupbox:AddLabel('Game: Arsenal')
    ...
    ThemeManager:SetFolder('NSHUB_Arsenal')
    ThemeManager:ApplyToTab(Tabs.Settings)
    
    -- UI Settings
    Library.ToggleKeybind = Options.MenuKeybind
    Library:SetWatermarkVisibility(true)
    Library:SetWatermark('NSHUB Arsenal | v1.0')
    ...
    LocalPlayer.CharacterAdded:Connect(function(character)
        task.wait(0.1)
        if Options.WalkSpeed and Options.WalkSpeed.Value ~= 18 then
            character:WaitForChild("Humanoid").WalkSpeed = Options.WalkSpeed.Value
        end
        if Options.JumpPower and Options.JumpPower.Value ~= 48 then
    ...
    -- Success Message
    task.wait(0.2)
    Library:Notify('NSHUB Arsenal loaded!', 5)
    Library:Notify('Press RightShift to toggle menu', 3)
    
    print('========================================')
    print('NSHUB Arsenal Script Loaded')
    print('Version: 1.0')
